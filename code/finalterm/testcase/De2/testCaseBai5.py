...
def run_testcase(spark, basePath, mssv, numQuestion, shiftNumber):
    ...
    if base.check_path_exists(check_data_path):
        totalpoint += 10
        if base.is_partitioned_path(check_data_path, spark, extension=extension, compression="snappy"):
            totalpoint += 10
            if base.check_schema_is_correct(check_data_path, base_path, spark, extension=extension, compression="snappy", partition_cols=["year", "month"]):
                totalpoint += 10
