...
def run_testcase(spark):
    ...
    file_list = list_files_in_github_folder(repo_owner, repo_name, github_folder_bai4)

    for file_path in file_list:
        save_path = f"{dbfs_base_path_bai4}/{os.path.basename(file_path)}"
        download_github_file(repo_owner, repo_name, file_path, save_path)

    base_path = f"""file:///dbfs{dbfs_base_path_bai4}"""
...
def check_json(path):
    """
    Kiểm tra tất cả file .json trong path có đang sử dụng Snappy compression hay không
    """
    dbfs_path = path.replace("file://", "") 
    files = os.listdir(dbfs_path)
    parquet_files = [f for f in files if f.endswith(".json")]
    if not parquet_files:
        print(f"❌ Không có file định dạng .json trong {path}")
        return False
    else:
        print(f"✅ Có file định dạng .json trong {path}!")
        return True
